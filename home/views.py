...
class TransacaoSucataView(APIView):
    ...
    def get(self, request, pk=''):
        if 'Sucata' in request.GET:
            ...
            serializer = TransacaoSucataSerializer(infoss, many=True)
            return Response(
                {
                    'data': serializer.data,
                    'total': 0,
            ...
            serializer = TransacaoSucataSerializer(infoss)
            return Response(
                {
                    'data': serializer.data,
                    'total': 0,
    ...
    def post(self, request):
        serializer = TransacaoSucataSerializer(data=request.data, many=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({"msg": "Inserido com sucesso"})
    ...
    def put(self, request, pk=''):
        infoss = TransacaoSucata.objects.get(id=pk)
        serializer = TransacaoSucataSerializer(infoss, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
